 ...
 * matching rows renders nothing unless it sets {@link description}. Registers with
...
export class BitRowGroupComponent<T = unknown> {
  ...
  /**
   * Explanatory text rendered under the header, above the group's rows — e.g. the
   * extension's "Save a login item for this site to autofill" tip. Setting it also
   * keeps the group visible when no rows match, so the text stands in for the rows.
   */
